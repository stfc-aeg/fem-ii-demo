...
class ipc_client:
    ...
    def run_req(self, run_once, msgType, msgVal, msgDevice, msgConfig):
        ...
        :param run_once: Boolean value, true when command line arguments were provided at runtime
...
def main():
    ...
    parser.add_argument("-url", "--url", help="Remote server url, default = tcp://localhost", 
    ...
    parser.add_argument("-led_config", "--led_config", help="LED device configuration option, accepts: %s " 
                        % LED_STATES, choices=LED_STATES)
    parser.add_argument("-temp_config", "--temp_config", help="Temperature device configuration option, accepts: %s " 
                        % TEMP_STATES, choices=TEMP_STATES)
    parser.add_argument("-power_config", "--power_config", help="Power device configuration option, accepts: %s " 
    ...
    if arg_length > 2 and arg_length < 5:
        ...
        # A proper message has been provided, set the one-shot flag and initialise a client
        run_once = True
        if arg_length == 2:
            run_once = False
        ...
        client.run_req(run_once, args.msg_type, args.msg_val, args.device, _config)
